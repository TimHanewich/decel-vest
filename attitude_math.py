...
def attitude(gx:float,gy:float,gz:float):
    ...
    # but it seems they have flipped the pitch and roll calculations. So I swapped them.
    pitch_rads = math.atan(ms2_y / math.sqrt((ms2_x * ms2_x) + (ms2_z * ms2_z)))
    roll_rads = math.atan(ms2_x / math.sqrt((ms2_y * ms2_y) + (ms2_z * ms2_z)))
